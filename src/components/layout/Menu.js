...
export default function Menu({active, setActive}) {
  return (
    <nav>
      ...
      <div>
        <button className="outline-none" onClick={() => setActive(!active)}><i className="material-icons text-xxl sm:text-heading md:hidden">{!active ? 'menu' : 'close'}</i></button>
        <StyledMenuDiv isActive={active} className={`absolute left-0 right-0 bg-dark-blue flex flex-col mt-4 md:hidden ${!active ? 'hidden' : ''}`}>
          <Link to={'/upcoming'} className="link-border font-medium p-3 text-base">Upcoming</Link>
          <Link to={'/previous'} className="link-border font-medium p-3 text-base">Previous</Link>
          <Link to={'/rockets'} className="link-border font-medium p-3 text-base">Rockets</Link>
        </StyledMenuDiv>
      </div>
    </nav>
  )
